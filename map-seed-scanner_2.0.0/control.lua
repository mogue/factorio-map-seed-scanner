...
local teleport_on_surface_deleted = function (e)
    game.create_surface("probe_surface", helpers.parse_map_exchange_string( storage.match_results[teleport_index] ).map_gen_settings)
...
local function teleport_to_map(match_index)
    if (match_index < 1 or match_index > #storage.match_results) then
...
local function stop()
    ...
    teleport_to_map(#storage.match_results)
...
local scan_on_surface_deleted = function(e)
    if ( analyzer.iterate(storage.map_gen_settings) ) then
        game.create_surface("probe_surface", storage.map_gen_settings)
    else
        stop()
    end
end
...
--	game.get_player(1).print("Scanning surface " .. e.surface_index .. " seed: " ..  storage.map_gen_settings.seed)
...
--	game.get_player(1).print("Chunk " .. e.surface.name .. " - " .. e.position.x .. "x" .. e.position.y )
    ...
    if e.position.x == gs.chunk_radius and e.position.y == gs.chunk_radius then
        ...
        if match_found then
            table.insert(storage.match_results, e.surface:get_map_exchange_string() )
        end
...
script.on_init(function ()
	...
	storage.settings = gs
    storage.match_results = default_match_results
	storage.map_gen_settings = util.copy(surf.map_gen_settings)
...
cmd["watch"] = function(command)
	watch = not watch
    game.get_player(1).print("Scanner watching: " .. tostring(watch))
end
...
cmd["matches_dump"] = function(command)
    logger.dump(
        "return " .. serpent.block(storage.match_results),
...
cmd["status"] = function(command)
	...
	game.get_player(1).print({"",
		...
		"Matches found: ", #storage.match_results, "\n",
